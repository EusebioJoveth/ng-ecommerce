...
@Component({
  ...
  imports: [QtySelector, CurrencyPipe, MatIconButton, MatIcon],
  template: `
    <div class="grid grid-cols-3 grid-cols-[3fr_1fr_1fr]">
      <div class="flex items-center gap-4">
        <img [src]="item()" alt="" class="w-24 h-24 rounded-lg object-cover">

        <div>
          <div class="text-gray-900 text-lg font-semibold">{{item().product.name}}</div>
          <div class="text-gray-600 text-lg">{{item().product.price | currency:' AO'}}</div>
        </div>
      </div>

      <app-qty-selector [quantity]="item().quantity" (qtyUpdate)="store.setItemQuantity({productId: item().product.id, quantity: $event})"/>
